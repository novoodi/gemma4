...
## 2026-07-14 — Reflection 자기비평 패스: 제약(싫어요) 준수 검증 (B④)

**결정**: 자율 검증 하네스에 두 번째 검증 계층 `ReflectionService`를 추가한다.
Guardrail(장소 실존, 하드 게이트) 옆에서, 추천이 사용자의 명시 제약(성향 프로필의
"싫어요:" 항목)을 위반하는지 결정론적으로 자기비평한다. 위반 시 재시도 피드백을 누적해
Gemini가 자가 수정하도록 유도한다.

**근거**: 프롬프트에 "선호/불호 엄격 반영"이 있으나 집행이 없었다 — PiiScrubber/Guardrail과
같은 "프롬프트는 1차 방어, 결정론적 게이트가 집행" 철학의 확장. 순수 Kotlin(JVM 테스트 가능).

**설계 판단**:
- **검사 대상은 실제 추천 항목(장소명+이유, 활동)뿐 — 요약문 제외**. 실기기 1차 검증에서
  Gemini가 요약에 "술집은 제외했어요"처럼 회피를 서술하자 그 토큰이 걸려 오탐(위반 5/5)이
  발생. 요약은 설명 산문이라 제약 대상이 아님 → 제외하니 준수한 재시도가 정상 통과.
- **정밀도 우선 매칭**: 불호 구절의 내용 토큰(불용어·단문자 제거)이 한 항목 안에 모두
  등장할 때만 위반. 단일 토큰·부정문 오탐 억제.
- **소프트 게이트 + 폴백**: Reflection은 재시도를 구동하되, 장소가 유효한(Guardrail 통과)
  결과는 폴백으로 보존 → 소프트 제약 미충족이 총 실패로 이어지지 않음. 하드 실패는
  여전히 Guardrail(존재하지 않는 장소)만.
- **불호 신호원은 구조화 프로필("싫어요:")만** — 자유 텍스트 후기의 의미 판정은 오탐이
  커 v1 제외(향후 온디바이스 LLM 시맨틱 판정으로 확장 여지). 후기는 여전히 프롬프트에 주입.

**실기기 검증(2026-07-14, SM-F966N)**: user_status에 "싫어요: 카페/디저트/…" 주입 후
마라탕 방 분석 → 시도1 위반 2건 → 시도2 위반 2건 → **시도3 위반 0건, Guardrail+Reflection
통과(총 3회)**. 자기비평→피드백→자가수정 루프가 카페/디저트를 배제하며 수렴함을 실증.

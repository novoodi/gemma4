...
> 단순 LLM 호출 래퍼가 아니라, **LLM 출력을 신뢰하지 않고 검증·교정하는 에이전트 시스템**과
> **온디바이스 AI 파이프라인**을 직접 설계·구현한 프로젝트입니다.

- **자율 검증 에이전트 하네스** — Gemini **Function Calling**(`getWeather`/`searchPlace` 툴 호출)을
  통제하는 오케스트레이터. 추천 결과를 **Guardrail**(장소 실존 팩트 체크)과 **Reflection**(사용자
  명시 제약 위반 자기비평) 2단으로 검증하고, 실패하면 피드백을 누적해 **최대 3회 자가 수정 재시도**한다.
  툴 사용 → 자기비평 → 재시도로 이어지는 agentic 루프를 밑바닥부터 구현.
- **온디바이스 프라이버시 방화벽** — 채팅 원문은 기기 밖으로 나가지 않는다. Gemma 4 온디바이스 익명화
  요약 + `PiiScrubber` 결정론적 마스킹 게이트(프롬프트 지시는 1차 방어, 집행은 규칙 기반 게이트).
- **온디바이스 시맨틱 RAG** — EmbeddingGemma-300m을 raw LiteRT + DJL 토크나이저로 직접 구동해 후기를
  기기 안에서 임베딩·검색(brute-force 코사인 top-k). 지난 모임 취향을 새 톡방 추천에 사용자 단위로 누적.
- **Constrained decoding** — 온디바이스 Gemma의 성향 압축 출력을 **툴 스키마로 문법 제약**(LiteRT-LM
  `enableConversationConstrainedDecoding`)해 깨진 JSON을 구조적으로 차단.

## 엔지니어링 성과 · 지표

- **온디바이스 임베딩 정확도(골든 테스트)** — Python `sentence-transformers` 기준 벡터 대비 코사인
  유사도 **min 0.9507**(임계 0.93), 쿼리별 랭킹 **top-1 5/5 일치**. 온디바이스 재현 정확도를 회귀 테스트로 보증.
- **자율 검증 하네스 실기기 실증** — Reflection이 사용자 불호("싫어요") 위반을 검출 → **3회 자가 수정으로
  수렴**(위반 2→2→0). Constrained decoding으로 성향 압축 **깨진 JSON 0건**.
- **Fail-open 제거** — 장소 검증 불가를 "검증됨"과 구분하는 **3-상태(OPEN/CLOSED/UNKNOWN)**로 분리해
  거짓 검증을 차단.
- **테스트 가능한 설계** — 핵심 로직(PII 스크러버·압축 파이프라인·리트리버·Reflection·직렬화)을
  Android 비의존 순수 Kotlin으로 분리 → **JVM 단위 테스트 50여 건**.

## 엔지니어링 규율

- **문서보다 코드 실측 우선** — 의존성 추가·아키텍처 변경은 본 구현 전 **스파이크(빌드 + 실기기 스모크)**로
  통과/실패/우회 기준을 명문화해 판정(예: LiteRT-LM + DJL 동시 상주 검증, constrained decoding 실기기 판정).
- **온디바이스 모델은 골든 테스트로 정확도 보증** — 임계값 미달 시 구현을 완료로 간주하지 않음.
- **온디바이스 모델 출력은 신뢰하지 않는다** — 방어적 JSON 파싱 + 결정론적 검증 게이트로 집행.
- 아키텍처 결정(채택/기각/방향 전환)은 `docs/DEVLOG.md`에 근거·기각 대안·잔여 리스크와 함께 기록.
